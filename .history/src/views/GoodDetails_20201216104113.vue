      ...
      <div class="content">
        ...
        <div class="productinfo">
          ...
          <div class="links-container" v-if="material.products && material.products.length > 0">
              <a href="##" v-for="item in material.products" @click.prevent="previewProduct(item.view)" :key="item.id" class="product"><i class="el-icon-document"></i> {{ item.title }}</a>
          </div>
          <div class="no-data" v-else>暂无产品规格说明书</div>
          pcn规格书：
          <div class="links-container" v-if="material.specificats && material.specificats.length > 0">
            <a  href="##" v-for="item in material.specificats" @click.prevent="previewSpecification(item.view)" :key="item.id" class="specification"><i class="el-icon-document"></i> {{ item.title }}</a>
          </div>
          <div class="no-data" v-else>暂无pcn规格书</div>
        ...
        <div class="material-item" v-for="material in materials" :key="material.id">
          <div class="dc-and-coo">
            DC(年份): <span >{{material.dc}}</span><pre>      </pre> COO(产地):<span >{{material.coo}}</span>   
          ...
          <div class="pictures">
            ...
            <div class="not-vip pictures-a">
              ...
              <ul class="materials">
                <li class="material" v-for="item in material.picturesA" :key="item.id"  ><img :src="item.path" @click="openAlbum(material.picturesA)"></li>
              </ul>
            </div>
            <div class="pictures-b" v-if="isHasB">
              <div class="title" >B类材料图片</div>
