        ...
        <div class="data"  v-if="brandList.length > 0">
          ...
          <el-table :data="product.list" stripe border header-row-class-name="product-table-header" v-loading="tableLoading" :cell-class-name="getCellClass">
            ...
            <el-table-column label="物料" align="center" min-width="100">
              ...
              <template slot-scope="scope"><a :href="href+'/details?brandName='+scope.row.brandName+'&materialNumber='+scope.row.materialNumber"   @click.stop="gotoGoodDetails(scope.row.brandName, scope.row.materialNumber)" >{{ scope.row.materialNumber }}</a></template>
            </el-table-column>
...
export default {
  ...
  data() {
    return {
      href:`${window.location.host}/dist/index.html#`,
