          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="80px">
            ...
            <el-form-item label="检测项" prop="checkeditems">
              <el-checkbox-group v-model="ruleForm.checkeditems"   >
                ...
                <el-checkbox label="X-ray" name="type" border></el-checkbox>
                <el-checkbox label="de-cap(开盖)" name="type" border></el-checkbox> -->
                <el-checkbox v-for="item in ruleForm.items" name="type" :key = "item.content" :disabled="item.disabled"  :label="item.content" border @change=""></el-checkbox>

              </el-checkbox-group>
            </el-form-item>
            <el-form-item label="是否寄回" prop="back">
              <el-radio-group v-model="ruleForm.back" size="medium" @change="backHandler">
                <el-radio border v-for="item in ruleForm.isback" :label="item.content" :disabled="item.disabled" :key="item.id"></el-radio>
            ...
            <el-form-item>
              <!-- <el-button type="primary" @click="onSubmit">提交委托测试</el-button> -->
               <el-button type="primary" @click="submitForm('ruleForm')">提交委托测试</el-button>
  ...
  data() {
    return {
      ...
      ruleForm: {
        ...
        checkeditems:[],
        items: [
          {id:0,disabled:false,content:"外观检测"},
          {id:1,disabled:false,content:"X-ray"},
          {id:2,disabled:false,content:"de-cap(开盖)"},
        ],

        back: '',
        isback:[
          {id:0,disabled:false,content:"不寄回"},
          {id:1,disabled:false,content:"寄回"},
        ],
        material: '',
        brand: '',
        // delivery: false,
  ...
  watch:{
    'ruleForm.back':{
      handler(newValue){
        ...
        if(newValue == '寄回'){
          ...
          var index =this.ruleForm.checkeditems.indexOf("de-cap(开盖)")
    ...
    'ruleForm.checkeditems':{
      handler(newValue){
        ...
        var index = newValue.indexOf("de-cap(开盖)")
        if(index != -1){
          this.ruleForm.isback[2].disabled=false
        }
      }
    }
  },
