      ...
      <div class="content">
        ...
        <div class="productinfo">
          ...
          <div class="links-container" v-if="material.products && material.products.length > 0">
              <a href="##" v-for="item in material.products" @click.prevent="previewProduct(item.view)" :key="item.id" class="product">{{ item.title }}</a>
          </div>
          <p class="no-data" v-else>暂无</p>
          pcn规格书：
          <div class="links-container" v-if="material.specificats && material.specificats.length > 0">
            <a  href="##" v-for="item in material.specificats" @click.prevent="previewSpecification(item.view)" :key="item.id" class="specification">{{ item.title }}</a>
          </div>
          <p class="no-data" v-else>暂无</p>
