      ...
      <div class="content">
        ...
        <div class="productinfo">
          ...
          <div class="links-container" v-if="material.products && material.products.length > 0">
              <a href="##" v-for="item in material.products" @click.prevent="previewProduct(item.view)" :key="item.id" class="product"><i class="el-icon-view"></i> {{ item.title }}</a>
          </div>
          <div class="no-data" v-else>暂无产品规格说明书</div>
          pcn规格书：
          <div class="links-container" v-if="material.specificats && material.specificats.length > 0">
            <a  href="##" v-for="item in material.specificats" @click.prevent="previewSpecification(item.view)" :key="item.id" class="specification"><i class="el-icon-view"></i> {{ item.title }}</a>
          </div>
