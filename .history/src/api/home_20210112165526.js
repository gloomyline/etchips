...
export default {
  ...
  async detect(payload) {
    const response = await axios.post('/chi/homePage/entrustCheck', JSON.stringify(payload));
    return response;
  },
  ...
  async cooperationConnector() {
    const response = await axios.get('/cooperationConnector/cooperationList');
    return response.data.cooperation;
  },
  //新闻动态
  async news() {
    const response = await axios.post('/journalismConnector/findLatest');
    return response.data.journalism;
  },
  //新闻动态
  async newsdetail(id) {
    const response = await axios.post('/journalismConnector/selectById',{id:id});
    return response.data.journalismByid;
  },
