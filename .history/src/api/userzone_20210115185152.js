export default {
  ...
  //查看所有发票
  async invoiceList(limit,page) {
    const response = await axios.get('/chi/invoice/findInvoiceList',{params: {
      limit:limit,
      page:page
    }})
    return response;
  ...
  //获取可以申请开发票订单列表
  async OrderList() {
    const response = await axios.get('/chi/order/invoiceOrderList')
    return response;
  },
  //申请开发票 参数  /chi/order/invoice（addressId,invoiceId,orderNumber）
  async applyInvoice(invoicelist) {
    const response = await axios.get('/chi/order/invoice',{params: {
      addressId:invoicelist.addressId,
      invoiceId:invoicelist.invoiceId,
      orderNumber:invoicelist.orderNumber
    }})
    return response;
  },
  ...
  //查询检测订单
  async getOrderList() {
    const response = await axios.get('/commissions/api/findAll',{params:{
      limit:100,
      page:1
    }})
    return response;
  ...
  async gotopay(payType,OrderNo) {
    const response = await axios.get('/commissions/api/payment',{params:{
      payType:payType,
      OrderNo:OrderNo,
  ...
  //委托检测取消订单
  async orderCancel(ids) {
    const response = await axios.get('/commissions/api/cancleOrderByIds',{params:{
      ids:ids
